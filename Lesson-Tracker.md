...
| **3E — Complete** | Connect POST to a request-scoped session and introduce a response schema that includes server-assigned fields. | `POST /runs` creates, adds, commits, refreshes, and returns a `Run` through `RunRead`. The success test queries the resulting row by primary key in `running_app_test`; all 4 tests pass. Learner explained request validation versus a table-mapped ORM object, commit, and test-database isolation. Retention check remains. |
| **3F — Complete** | Trigger a duplicate-ID failure; define the initial API conflict policy and session cleanup. | The duplicate returns a safe `409`, the original row remains, and a later valid request succeeds through the same session. All 5 tests pass. Learner explained database detection, transaction reset, and conflict semantics. Retention check remains. |
| **5A — Complete** | Retrieve one saved run through `GET /runs/{run_id}` and return a stable missing-resource response. | Existing IDs return `200` through `RunRead`; missing integer IDs return `404` with a stable message. Pytest reports **7 passed**. Learner explained read-only behavior and integer path validation versus missing database rows. Retention check remains. |
| **5B — Current** | List saved runs with explicit, stable ordering before adding pagination. | Next: create two runs, request `GET /runs`, and define the expected list order. Completion requires a database-backed list with deterministic ordering. |
...
- Route dependency evidence: `create_run()` now declares `db: Session = Depends(get_db)`. Pytest still reports **4 passed**, showing the dependency can be resolved through the isolated test override while the route continues to return the original validated request. Learner correctly identified test-database isolation; clarified that the tests also remain green because the route's response behavior has not changed yet.
- Persistence-contract test evidence: after the success test began requiring database-generated `id`, `created_at`, and `updated_at`, it failed with `KeyError: 'id'` as predicted. This confirms the test detects the route's missing persistence/response behavior. The same run exposed an unrelated `NameError` because the zero-distance test calls `db_client` without declaring the fixture parameter; restore that parameter before proceeding.
- Focused red-test evidence: restoring `db_client` to the zero-distance test removed the unrelated failure. Pytest now reports **1 failed, 3 passed**; only the expected `KeyError: 'id'` remains. The suite is ready to guide the route-persistence implementation.
- Route-persistence evidence: `POST /runs` converts `RunCreate` to `Run`, adds and commits it through the injected session, refreshes database-generated fields, and serializes it through `RunRead`. The former `id` failure is resolved, and assertions for integer ID plus creation/update timestamps pass. The sole remaining failure compares timestamp text: `2026-01-01T00:00:00Z` and `2025-12-31T16:00:00-08:00` represent the same instant, so the test must compare parsed timezone-aware datetime values.
- Time-comparison verification: the test now parses both timestamp strings into aware `datetime` values before comparison. Pytest reports **4 passed** with the known TestClient/httpx deprecation warning. This verifies the HTTP response path and teaches that equal instants can have different date/time text when their UTC offsets differ.
- Direct-persistence verification: the success test receives the same isolated `db_session` used by the overridden route dependency, loads the new `Run` by its response ID, and verifies the stored values. Pytest reports **4 passed** with the known warning. Lesson 3E is implemented and verified; learner teach-back remains before completion.
- Lesson 3E teach-back checkpoint: learner correctly explained that `commit()` saves the transaction and that `running_app_test` prevents tests from polluting the normal application database. Clarification remains for why validated `RunCreate` data must become the table-mapped `Run` object and why `refresh()` reloads database-generated/current column values after saving.
- Lesson 3E completion: learner explained that SQLAlchemy saves `Run` because it is the model mapped to the real database table. Clarified that `RunCreate` validates incoming data, while `refresh()` explicitly reloads the database's current/generated values after commit. Implementation, verification, and explanation are complete; retention remains for a later lesson.
- Lesson 3F initial prediction: learner recognized that a first saved row is needed before a second request can demonstrate a duplicate. Clarified that `RunCreate` sees only one request and has no database knowledge, while PostgreSQL checks uniqueness against stored/concurrent rows. The test uses the real `runs` table in `running_app_test` inside a rollback-controlled transaction, not a temporary table.
- Duplicate-failure evidence: the first duplicate test produced PostgreSQL `UniqueViolation` for constraint `runs_external_activity_id_key`, surfaced by SQLAlchemy as `IntegrityError`. Pytest reports **1 failed, 4 passed**. This verifies the database is enforcing uniqueness; the API still needs to roll back the failed transaction and translate the internal exception into a stable `409` response.
- Conflict-response verification: the route now catches `IntegrityError`, rolls back the failed transaction, and returns `409 Conflict` with `{"detail": "Run already exists"}`. Pytest reports **5 passed** with the known warning. Remaining 3F evidence: confirm the first row remains after the duplicate rollback and that another valid request can use the session.
- Fixture-injection debugging: while extending the duplicate workflow, importing `db_session` directly from `tests.conftest` produced a Pytest `FixtureFunctionDefinition`, so calling `.get()` raised `AttributeError`. Fixtures are discovered from `conftest.py` automatically; each test that needs the yielded SQLAlchemy session must declare `db_session: Session` as a parameter instead of importing the fixture function.
- Lesson 3F teach-back checkpoint: learner was unsure which layer detects duplicates and why `409` applies, and initially described rollback as preventing corrupt table input. Clarified that PostgreSQL's unique constraint detects the collision, SQLAlchemy reports it, and the route translates it; the rejected row is never saved. `rollback()` resets the failed transaction so later work can proceed, while `409` communicates that valid input conflicts with existing database state.
- Duplicate-recovery verification: after requesting `db_session` through the test parameter instead of importing the fixture definition, the duplicate workflow passes. It verifies the original committed row still exists after the failed duplicate is rolled back and verifies another unique request succeeds through the same session. Pytest reports **5 passed** with the known warning. Lesson 3F is implemented and verified; teach-back remains.
- Lesson 3F completion: learner explained that PostgreSQL detects the duplicate, rollback resets the session, and `409` represents a conflict with an existing item. Refined “existing request” to “existing row/resource.” Implementation, verification, and explanation are complete; retention remains for a later lesson.
- Lesson 5A red-test evidence: a POST successfully creates a run and returns its ID, but `GET /runs/{id}` returns FastAPI's default `404` because no matching route exists. Pytest reports **1 failed, 5 passed**. This isolates the next behavior: retrieve one `Run` by primary key.
- Lesson 5A retrieval verification: `GET /runs/{run_id}` now accepts an integer path parameter, uses `db.get(Run, run_id)` to search by primary key, and returns the ORM row through `RunRead`. The create-then-get workflow passes; Pytest reports **6 passed** with the known warning. The missing-row branch exists but still needs a direct test.
- Lesson 5A missing-row verification: requesting a well-formed integer ID with no corresponding row returns `404` and `{"detail": "Run not found"}`. Pytest reports **7 passed** with the known warning. Existing and missing retrieval paths are implemented and verified; learner teach-back remains.
- Lesson 5A completion: learner explained that GET does not commit because it retrieves rather than stores data. Clarified that `/runs/999999999` contains a valid integer and reaches the database lookup before returning `404`, while `/runs/not-a-number` is URL text that FastAPI cannot convert to the declared `int`, so validation returns `422` before the route executes. Implementation, verification, and explanation are complete; retention remains.
...
- [x] Create and persist a valid run. (The API test verifies generated response fields and queries the saved row by primary key in the isolated test database.)
...
- [x] Retrieve an existing run.
- [x] Return `404` for a missing run.
